...
fmri = pd.read_csv("Real_Numerical_result.csv",header=0)
fmri = fmri[~fmri["Encoding Algorithm"].isin(["BUFF", "CHIMP"])]# Remove unsupported encodings BUFF and CHIMP
# print(fmri.sample(3))
...
nnr = pd.read_csv("result_compression_ratio.csv",header=0)
...
gzip=fmri[fmri["Compress Algorithm"]=="GZIP"] #Compress -> Compress Algorithm
gzip_means = gzip.groupby(['Encoding Algorithm'])[["Encoding Time","Compress Time","Uncompress Time","Decoding Time","Compression Ratio"]].mean() #Encoding -> Encoding Algorithm and [[....]]
# print(gzip_means)

print("Encodings in Java results:", gzip_means.index.tolist())
print("Encodings with CR data:", gzip_nnr_nor.index.tolist())
...
# print(gzip_nor)
gzip_nor=gzip_nor.sort_values('Encoding Algorithm')#Encoding -> Encoding Algorithm
...
lz4=fmri[fmri["Compress Algorithm"]=="LZ4"]
lz4_means = lz4.groupby(['Encoding Algorithm'])[["Encoding Time","Compress Time","Uncompress Time","Decoding Time","Compression Ratio"]].mean()
...
lz4_nor=lz4_nor.sort_values('Encoding Algorithm')
...
snappy=fmri[fmri["Compress Algorithm"]=="SNAPPY"]
snappy_means = snappy.groupby(['Encoding Algorithm'])[["Encoding Time","Compress Time","Uncompress Time","Decoding Time","Compression Ratio"]].mean()
...
snappy_nor=snappy_nor.sort_values('Encoding Algorithm')
...
#fmri = pd.read_csv("radar/new_UncompressedSpeedResult.csv",header=0) #remove
uncompressed=fmri[fmri["Compress Algorithm"]=="UNCOMPRESSED"]
uncompressed_means = uncompressed.groupby(['Encoding Algorithm'])[["Encoding Time","Compress Time","Uncompress Time","Decoding Time","Compression Ratio"]].mean()
...
uncompressed_nor=uncompressed_nor.sort_values('Encoding Algorithm')
